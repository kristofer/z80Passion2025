...
#include <string.h>
//#include <unistd.h>
//#include <termios.h> // Add this for terminal control
#include <agon/vdp_vdu.h>

// Add VT100 escape sequence definitions
#define ESC "\033"
...
#define VT100_STANDOUT_ON      ESC "[7m"
#define VT100_STANDOUT_OFF     ESC "[0m"
...
// Add these after existing #define statements
//struct termios orig_termios;  /* Original terminal state */
int LINES = 24;    /* Terminal rows - replace curses constant */
int COLS = 80;     /* Terminal columns - replace curses constant */
...
// Forwards
/* VT100 terminal handling functions */
//void vdp_console_mode(int on);  /* From Agon VDP library */
void term_init(void);
...
/* Initialize the terminal in VT100 mode */
void term_init() {
    /* Enter VT100 console mode */
    vdp_console_mode(1);
    ...
    /* For Agon, the screen size would typically be 40x25 or 80x25 */
    LINES = 25;
    COLS = 80;
    
    /* Define MSGLINE based on LINES */
    #undef MSGLINE
    #define MSGLINE (LINES-1)
}
...
void move(int y, int x) {
    printf(VT100_CURSOR_POSITION, y + 1, x + 1);
}
...
/* Get a character from keyboard with escape sequence handling */
int getch() {
    int c;
    char seq[4];
    ...
    if (c == '\033') {
        /* Read up to 3 more bytes of the escape sequence */
        ...
        if (seq[0] == '[') {
            if (seq[1] >= '0' && seq[1] <= '9') {
                ...
                if (seq[2] == '~') {
                    switch (seq[1]) {
                        case '3': return '\033'; /* DEL key - needs mapping to editor sequence */
                ...
                switch (seq[1]) {
                    case 'A': return '\033'; /* Up - needs mapping */
                    case 'B': return '\033'; /* Down - needs mapping */
                    case 'C': return '\033'; /* Right - needs mapping */
                    case 'D': return '\033'; /* Left - needs mapping */
                    case 'H': return '\033'; /* Home - needs mapping */
                    case 'F': return '\033'; /* End - needs mapping */
                }
            }
        }
        
        /* Return the escape sequence as is */
        ungetc(seq[1], stdin);
        ungetc(seq[0], stdin);
        return '\033';
    }
