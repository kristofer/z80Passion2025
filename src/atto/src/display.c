...
void display(window_t *wp, int flag)
{
	...
	ktbox_move_cursor(box, 0, wp->w_top);
	...
	if (wp == curwp && flag) {
		...
		ktbox_move_cursor(box, bp->b_col, bp->b_row);
		//refresh();
	}
	//curs_set(1);
...
void modeline(window_t *wp)
{
	...
	ktbox_move_cursor(box, 0, wp->w_top + wp->w_rows);
...
{
	//move(MSGLINE, 0);
	ktbox_move_cursor(box, 0, MSGLINE);

	if (msgflag) {
		//attron(COLOR_PAIR(ID_SYMBOL));
		//addstr(msgline);
		ktbox_addstr(box, msgline);
		msgflag = FALSE;
	}
	//blanktoeol();
	ktbox_clrtoeol(box);
...
void display_prompt_and_response(char *prompt, char *response)
{
	...
	ktbox_puts(box, 0, MSGLINE, prompt);
	/* if we have a value print it and go to end of it */
	if (response[0] != '\0')
		//addstr(response);
		ktbox_addstr(box, response);
	//blanktoeol();
	ktbox_clrtoeol(box);
}
...
void update_display()
{
	...
	ktbox_move_cursor(box, curwp->w_row, curwp->w_col);
