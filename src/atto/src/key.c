/* key.c, Atto Emacs, Public Domain, Hugh Barney, 2016, Kristofer Younger, 2025 (AGON), Derived from: Anthony's Editor January 93 */
...
//	{"esc esc show-version     ", "\x1B\x1B", version },
	...
	// AGON keys
	{"up previous-line         ", "\x0A", up },
	{"down next-line           ", "\x0B", down },
	{"left backward-character  ", "\x08", left },
	{"right forward-character  ", "\x0C", right },
	//
...
// get input - get a line of input from the user during a modeline prompt
// it handles some simple line editing
int getinput(char *prompt, char *buf, int nbuf, int flag)
{
	...
	ktbox_puts(box, 0, MSGLINE, prompt); // should this move editor/screen cursor?
	//blanktoeol();
	ktbox_fill_region(box, cpos, MSGLINE, COLS, MSGLINE, ' ');
	...
	for (;;) {
		...
		//case 0x08: /* backspace */
			...
			// addch(' ');
			// move(MSGLINE, start_col + cpos - 1);
			ktbox_putchar(box, start_col + cpos - 1, MSGLINE, ' ');
			buf[--cpos] = '\0';
			break;
		...
		default:
			if (cpos < nbuf -1) {
				//addch(c);
				ktbox_putchar(box, start_col + cpos, MSGLINE, c);
				buf[cpos++] = c;
				buf[cpos] ='\0';
			}
			break;
