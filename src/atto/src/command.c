/* command.c, Atto Emacs, Public Domain, Hugh Barney, 2016, Kristofer Younger, 2025 (AGON), Derived from: Anthony's Editor January 93 */
...
void quit_ask()
{
	if (modified_buffers() > 0) {
		...
		ktbox_puts(box, 0, MSGLINE, "Modified buffers exist; really exit (y/n) ?");
		//blanktoeol();
		ktbox_clrtoeol(box);
		if (!yesno(FALSE))
			return;
...
void killbuffer()
{
	...
	if (curbp->b_flags & B_MODIFIED) {
		...
		ktbox_puts(box, 0, MSGLINE, "Discard changes (y/n) ?");
		//blanktoeol();
		ktbox_clrtoeol(box);
		if (!yesno(FALSE))
			return;
...
char *unctrl(int ch) {
     ...
     if (ch < 32) {
         return sprintf("^%c", ch + 64);  // Show as ^A, ^B, etc.
     } else if (ch == 127) {
